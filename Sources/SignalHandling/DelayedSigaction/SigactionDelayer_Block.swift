...
/**
 ...
 any signals you want, including those you won’t need to delay, but to avoid
 some potentially nasty side-effects, you should only ever bootstrap the signals
 you will need to delay.
 ...
 re-send the signal to our thread! In which case we lose the info in
 `siginfo_t`, and a thread is stuck with a pending signal forever… (I have
 actually not observed another behavior on macOS.)
 ...
 sigaction prior to registering the signal source, then setting it back after
 the registration of the source fixes the issue though, so we did that. This
 solution seems weak though, and might break in the future.
...
public enum SigactionDelayer_Block {
	...
	/**
	 Prepare the process for delayed sigaction by blocking all signals on the
	 main thread and spawning a thread to handle signals.
	 
	 Delayed sigaction is done by blocking delayed signals until the time has
	 come to call the sigaction handler.
	 If any thread does not block the delayed signal, the sigaction will be
	 called before its time!
	 
	 - Note: How do we know a signal has arrived if it is blocked? We use
	 libdispatch to be notified when a new signal arrives. libdispatch uses
	 `kqueue` on BSD and `signalfd` on Linux. Signals are still sent to `kqueue`
	 and `signalfd` when they are blocked, so it works.
	 
	 - Important: Must be called before any thread is spawned.
	 - Important: You should not use `pthread_sigmask` to sigprocmask (nor
	 anything to unblock signals) after calling this method. */
